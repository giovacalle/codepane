...
export function createEditorStore(rootPath: string) {
  ...
  const store = createStore<EditorStoreState>((set, get) => {
    ...
    setActiveTab(tabId: string) {
      ...
      if (savedPos) {
        set({ activeTabId: tabId, selectedPath: tab.path, cursorLine: savedPos.line, cursorCol: savedPos.col });
      } else {
    ...
    async replaceOne(path: string, match: { line: number; column: number; length: number }, replaceText: string) {
      if (!adapter) return;
    ...
    saveCursorPosition(path: string, position: { line: number; col: number; scrollTop?: number }) {
