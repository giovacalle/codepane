...
import { createMemoryConfigAdapter, createLocalStorageAdapter } from '../../src/core/config-storage'
...
describe('createLocalStorageAdapter', () => {
  ...
  it('uses "editor" as the default prefix', async () => {
    ...
    expect(localStorage.getItem('editor:theme')).toBe(JSON.stringify({ dark: true }))
  })
  ...
  it('supports a custom prefix', async () => {
    ...
    expect(localStorage.getItem('my-app:layout')).toBe(JSON.stringify({ panel: 50 }))
  })
